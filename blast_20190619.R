...
ramdisk=0 # switch ramdisk usage on (1) and off (0)
...
if(ramdisk==1){
dbdir=args[3] # location of the sequences data bases  (RAMDISK VERSION) 
}else{
dbdir=wdir  # location of the sequences data bases  (NO RAMDISK VERSION)
}

system(paste0("echo 'BEGINNING OF PIECE ",inputf,": ", Sys.time(),"' >> ../Rreport.txt"))  
...
if(length(tmp2)>1){
rec.id.tmp=tmp2[,2] # first item is the pattern, second item is the requested part of the pattern
}
}
return(rec.id.tmp)
}
...
if(as.numeric(tmp[1,1])<=0.01){ # threshold to avoid bogus hits
...
} ## end of if(as.numeric(tmp[1,1])<=0.01)
...
ortho.blast.WGS <- function(mouse_gene_id,mouse_prot_id, species_id) {  # script to find 1:1 orthologue sequence by doing a reciprocal blast (tblastn,blastp) on whole-genome sequencing data from Ensembl or NCBI repository
 ...
 seq_name="no_ortho" # in case no orthologue present even on transcriptome sequencing data
 seq_id="" # intermediary variable
# step1: tblastn against Ensembl/NCBI predicted protein sequences from WGS genomes - command is killed after 45 seconds (in case of low complexity sequences which create endless queries)
cmd=paste0("system(\"timeout 45s ",wdir, "/ncbi-blast-2.8.1+/bin/tblastn -db ",dbdir,"/db2/nt/",species_id,"_WGS -query ../",mouse_prot_id,".fasta -out ../tmp_",mouse_prot_id,".txt -num_threads 3 -outfmt '6 evalue saccver stitle sseq' -max_target_seqs 1\")") # tblastn to query WGS data from Ensembl or NCBI of target species protein seqs 
eval(parse(text=cmd))
...
res=read.table("xaa_1_100_presence",h=F,row.names=1,stringsAsFactors = FALSE) 
res2=read.table("xaa_1_100_names",h=F,row.names=1,stringsAsFactors = FALSE) 
res=read.table("xaa_presence",h=F,row.names=1,stringsAsFactors = FALSE) 
res2=read.table("xaa_names",h=F,row.names=1,stringsAsFactors = FALSE)
}
...
res_table=foreach(i=1:nrow(res2), .combine='rbind',.packages=c("stringr")) %dopar% { # too slow...ramdisk?
...
system(paste0("echo 'END OF PIECE ",inputf,": ", Sys.time(),"' >> ../Rreport.txt"))  
